...
백엔드에서 서명과 검증을 담당하기 때문에 주로 프론트엔드를 맡았던 나는 인증할 때 사용하는 토큰이라고만 알고 사용하고 있었는데, 자주 사용하는만큼 정확하게 알아야겠다는 생각이 들어서 RFC 문서를 읽으면서 정리해보게 되었다.
...
- sub: JWT 주체인 개체 식별, 발급자의 컨텍스트에서 로컬로 고유하거나 전역적으로 고유한 값이어야 한다. user_id처럼 발급 서비스 내에서만 고유하면 되는 값이거나, 이메일 주소처럼 전역적으로 고유한 값을 쓴다.
- iss: JWT 발급 주체 식별
...
JWT를 사용하는 사용자가 자유롭게 정의할 수 있다. 충돌 방지를 위해 **URI 형식**으로 클레임 네임을 사용해서 다른 서비스와 이름이 중복되지 않는다.
...
JWT를 만드는 쪽(서버)과 쓰는 쪽(다른 서버/클라이언트)이 서로 '이 필드는 이런 뜻으로 쓰자'고 미리 약속만 하면, 표준에 없는 클레임 이름을 자유롭게 만들어 써도 된다.
